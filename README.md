...
- Полноэкранный терминальный интерфейс на `ncurses` в единой структуре лабораторных работ:
  меню стрелками, окно состояния, окно результата, footer с подсказкой и fallback в обычный числовой режим без TTY.
...
`make run` запускает полноэкранный интерфейс. Если программа запущена без интерактивного терминала, автоматически используется обычное числовое меню.
